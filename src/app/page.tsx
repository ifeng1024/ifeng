  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 w-full max-w-md">
        <h1 className="text-xl sm:text-2xl font-bold text-center text-gray-800 mb-2">食堂管理系统</h1>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow p-4 sm:p-5">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => { setShowForm(!showForm); setEditingId(null); setForm({ company_name: '', username: '', real_name: '', password: '', expires_at: '' }); }} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm">{showForm && !editingId ? '取消' : '新建账号'}</button>
        </div>
...
function KpiCard({ label, value, color, small }: { label: string; value: string; color: string; small?: boolean }) {
  ...
  return (
    <div className={`${colorMap[color] || 'bg-gray-50 text-gray-700'} rounded-xl ${small ? 'p-3' : 'p-4 sm:p-5'}`}>
      <div className={`${small ? 'text-xs' : 'text-sm'} opacity-70`}>{label}</div>
      <div className={`${small ? 'text-lg' : 'text-xl sm:text-2xl'} font-bold mt-1`}>{value}</div>
    </div>
  );
}
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<ApiResp>(`/api/dashboard?date=${today}&range=7d`).then(res => {
...
function RevenuePage({ user }: { user: AuthUser }) {
  ...
  const exportRevenueExcel = async () => {
    ...
    const ws = XLSXMod.utils.json_to_sheet(data); const wb = XLSXMod.utils.book_new(); XLSXMod.utils.book_append_sheet(wb, ws, '营收'); XLSXMod.writeFile(wb, '营收记录.xlsx');
  };
  ...
  const submit = async () => {
    ...
    if (editingId) {
      ...
      if (res.success) { setEditingId(null); setForm(f => ({ ...f, order_count: '', amount: '', note: '' })); loadRecords(); } else alert(res.error);
    } else {
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-white rounded-xl shadow p-4 sm:p-5">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => exportRevenueExcel()} className="bg-green-600 text-white px-3 py-1.5 rounded-lg text-sm">导出Excel</button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="sm:col-span-2 lg:col-span-3"><label className="block text-xs font-medium text-gray-600 mb-1">备注</label><input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} className="w-full border rounded-lg px-3 py-2 text-sm" /></div>
            <div className="sm:col-span-2 lg:col-span-3 flex gap-2"><button onClick={submit} className="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm">{editingId ? '保存' : '录入'}</button>{editingId && <button onClick={() => setEditingId(null)} className="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg text-sm">取消编辑</button>}</div>
          </div>
      </div>
...
function ExpensePage() {
  ...
  const [form, setForm] = useState({ canteen_id: '', stall_id: '', expense_date: new Date().toLocaleDateString('sv-SE'), category: '食材采购', amount: '', note: '', repeat_start_date: '', repeat_end_date: '', product_category_id: '', product_id: '', product_spec_id: '', quantity: '', unit_price: '', supplier_id: '' });
  const [editingId, setEditingId] = useState<string | null>(null);
  ...
  const submit = async () => {
    ...
    const body: Record<string, unknown> = { canteen_id: form.canteen_id, stall_id: form.stall_id || null, expense_date: form.expense_date, category: form.category, amount: num(form.amount), note: form.note };
    if (form.repeat_start_date && form.repeat_end_date) {
      body.repeat_start_date = form.repeat_start_date;
      body.repeat_end_date = form.repeat_end_date;
    }
    if (isMaterial) {
    ...
    if (res.success) { setForm(f => ({ ...f, amount: '', note: '', product_category_id: '', product_id: '', product_spec_id: '', quantity: '', unit_price: '', repeat_start_date: '', repeat_end_date: '', supplier_id: '' })); loadRecords(); } else alert(res.error);
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-white rounded-xl shadow p-4 sm:p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <label className="block text-xs font-medium text-gray-600 mb-1">重复时间</label>
            <div className="flex items-center gap-2">
              <input type="date" value={form.repeat_start_date} onChange={e => setForm(f => ({ ...f, repeat_start_date: e.target.value }))} className="w-full border rounded-lg px-2 py-2 text-sm" />
              <span className="text-gray-400 text-sm shrink-0">至</span>
              <input type="date" value={form.repeat_end_date} onChange={e => setForm(f => ({ ...f, repeat_end_date: e.target.value }))} className="w-full border rounded-lg px-2 py-2 text-sm" />
            </div>
            <p className="text-xs text-gray-400 mt-1">留空则不重复，填写后此期间每天自动产生一条支出</p>
          </div>
          <div className="sm:col-span-2 lg:col-span-3"><label className="block text-xs font-medium text-gray-600 mb-1">备注</label><input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} className="w-full border rounded-lg px-3 py-2 text-sm" /></div>
          {isMaterial && <>
            ...
            <tbody>
              {records.map(r => (
                <tr key={r.id as string} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 text-right">{fmt(r.amount)}</td>
                  <td className="px-3 py-2">{r.repeat_start_date && r.repeat_end_date ? <span className="text-xs text-blue-500">{String(r.repeat_start_date).slice(5)}~{String(r.repeat_end_date).slice(5)}</span> : ''}</td>
      ...
      {editingId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-5 sm:p-6 w-full max-w-md space-y-4">
            ...
            <div><label className="text-sm text-gray-600">类别</label><select value={String(editForm.category ?? '')} onChange={e => setEditForm(f => ({...f, category: e.target.value}))} className="w-full border rounded px-3 py-2 mt-1"><option>食材采购</option><option>人工成本</option><option>水电费</option><option>房租</option><option>设备维护</option><option>其他</option></select></div>
...
/* ─── SETTINGS PAGE ─── */
type SettingsTab = 'my_account' | 'canteens' | 'stalls' | 'meal_types' | 'accounts' | 'products' | 'suppliers';
...
function SettingsPage({ user }: { user: AuthUser }) {
  ...
  if (isCompanyManager) {
    tabs = tabs.concat([
      ...
      { key: 'meal_types', label: '餐别管理' }, { key: 'accounts', label: '账号管理' }, { key: 'products', label: '商品管理' }, { key: 'suppliers', label: '供应商管理' },
    ]);
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1 mb-6 border-b pb-2 overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`px-3 sm:px-4 py-2 rounded-t text-sm font-medium transition whitespace-nowrap ${tab === t.key ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>{t.label}</button>
        ))}
      </div>
...
function StallManagerSection() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center"><h3 className="font-semibold text-gray-700">档口管理</h3><button onClick={() => { setShowForm(!showForm); setEditingId(null); setForm({ name: '', manager_id: '', note: '' }); }} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm">{showForm && !editingId ? '取消' : '新增档口'}</button></div>
...
function MealTypeManager() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center"><h3 className="font-semibold text-gray-700">餐别管理</h3><button onClick={() => setShowForm(!showForm)} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm">{showForm ? '取消' : '新增餐别'}</button></div>
      {showForm && (
        <div className="bg-white rounded-xl shadow p-4 sm:p-5">
...
function AccountManager({ user: currentUser }: { user: AuthUser }) {
  ...
  const [form, setForm] = useState({ username: '', real_name: '', role_code: defaultRole, org_id: '', password: '', canteen_ids: [] as string[], stall_ids: [] as string[] });
  ...
  const loadStalls = useCallback(async () => {
    if (isCompanyManager && form.canteen_ids.length > 0) {
      // Load stalls for selected canteens
      const allStalls: Record<string, unknown>[] = [];
      for (const cid of form.canteen_ids) {
        const res = await apiFetch<ApiResp>(`/api/dropdown/stalls?canteen_id=${cid}`);
        if (res.success && res.data) allStalls.push(...(res.data as Record<string, unknown>[]));
      }
      setStalls(allStalls);
    } else {
  ...
  }, [isCompanyManager, form.canteen_ids]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { loadStalls(); }, [loadStalls]);

  // Filter stalls based on selected canteens for STALL_MANAGER
  const availableStalls = isCompanyManager && form.role_code === 'STALL_MANAGER' && form.canteen_ids.length > 0
    ? stalls.filter(s => form.canteen_ids.includes(s.canteen_id as string))
    : stalls;
  ...
  const submit = async () => {
    ...
    if (editingId) {
      ...
      if (form.password) body.password = form.password;
      const res = await apiFetch<ApiResp>(`/api/users/${editingId}`, { method: 'PUT', body: JSON.stringify(body) });
      if (res.success) { setShowForm(false); setEditingId(null); setForm({ username: '', real_name: '', role_code: defaultRole, org_id: '', password: '', canteen_ids: [], stall_ids: [] }); load(); } else alert(res.error);
    } else {
      ...
      if (form.role_code === 'CANTEEN_MANAGER' && form.canteen_ids.length > 0) {
        body.canteen_ids = form.canteen_ids;
      }
      if (form.role_code === 'STALL_MANAGER' && form.stall_ids.length > 0) {
        body.stall_ids = form.stall_ids;
      }
      const res = await apiFetch<ApiResp>('/api/users', { method: 'POST', body: JSON.stringify(body) });
      if (res.success) { setShowForm(false); setForm({ username: '', real_name: '', role_code: defaultRole, org_id: '', password: '', canteen_ids: [], stall_ids: [] }); load(); } else alert(res.error);
    }
  };

  const startEdit = (u: Record<string, unknown>) => {
    setEditingId(u.id as string);
    setForm({ username: u.username as string || '', real_name: u.real_name as string || '', role_code: u.role_code as string || defaultRole, org_id: u.org_id as string || '', password: '', canteen_ids: [], stall_ids: [] });
    setShowForm(true);
  };
  ...
  const toggleCanteen = (id: string) => {
    setForm(f => ({
      ...f,
      canteen_ids: f.canteen_ids.includes(id) ? f.canteen_ids.filter(x => x !== id) : [...f.canteen_ids, id],
      stall_ids: [],
    }));
  };
  ...
  const toggleStall = (id: string) => {
    setForm(f => ({
      ...f,
      stall_ids: f.stall_ids.includes(id) ? f.stall_ids.filter(x => x !== id) : [...f.stall_ids, id],
    }));
  };

  // Get org display name for user
  const getOrgName = (u: Record<string, unknown>) => {
    if (u.role_code === 'CANTEEN_MANAGER') return u.canteen_name as string || '-';
    if (u.role_code === 'STALL_MANAGER') return u.stall_name as string || '-';
    if (u.role_code === 'COMPANY_MANAGER') return u.company_name as string || '-';
    return '-';
  };

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center"><h3 className="font-semibold text-gray-700">账号管理</h3><button onClick={() => { setShowForm(!showForm); setEditingId(null); setForm({ username: '', real_name: '', role_code: defaultRole, org_id: '', password: '', canteen_ids: [], stall_ids: [] }); }} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm">{showForm && !editingId ? '取消' : '新增账号'}</button></div>
      {showForm && (
        <div className="bg-white rounded-xl shadow p-4 sm:p-5">
          <h4 className="font-medium text-gray-600 mb-3">{editingId ? '编辑账号' : '新增账号'}</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div><label className="block text-xs font-medium text-gray-600 mb-1">用户名 {!editingId && '*'}</label><input value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} disabled={!!editingId} className="w-full border rounded-lg px-3 py-2 text-sm disabled:bg-gray-100" /></div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">姓名 *</label><input value={form.real_name} onChange={e => setForm(f => ({ ...f, real_name: e.target.value }))} className="w-full border rounded-lg px-3 py-2 text-sm" /></div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">角色</label><select value={form.role_code} onChange={e => setForm(f => ({ ...f, role_code: e.target.value, org_id: '', canteen_ids: [], stall_ids: [] }))} className="w-full border rounded-lg px-3 py-2 text-sm">{roleOptions.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}</select></div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">{editingId ? '新密码（留空不改）' : '密码 *'}</label><input type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} className="w-full border rounded-lg px-3 py-2 text-sm" /></div>
          </div>

          {/* Multi-select canteens for CANTEEN_MANAGER */}
          {form.role_code === 'CANTEEN_MANAGER' && !editingId && (
            <div className="mt-4">
              <label className="block text-xs font-medium text-gray-600 mb-2">选择食堂 *（可多选）</label>
              <div className="flex flex-wrap gap-2">
                {canteens.map(c => (
                  <button key={c.id as string} type="button" onClick={() => toggleCanteen(c.id as string)} className={`px-3 py-1.5 rounded-lg text-sm border transition ${form.canteen_ids.includes(c.id as string) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:border-blue-300'}`}>{c.name as string}</button>
                ))}
                {canteens.length === 0 && <span className="text-gray-400 text-sm">暂无可选食堂</span>}
              </div>
            </div>
          )}

          {/* Multi-select stalls for STALL_MANAGER */}
          {form.role_code === 'STALL_MANAGER' && !editingId && (
            <div className="mt-4 space-y-3">
              {isCompanyManager && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-2">先选择食堂（筛选档口）</label>
                  <div className="flex flex-wrap gap-2">
                    {canteens.map(c => (
                      <button key={c.id as string} type="button" onClick={() => toggleCanteen(c.id as string)} className={`px-3 py-1.5 rounded-lg text-sm border transition ${form.canteen_ids.includes(c.id as string) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:border-blue-300'}`}>{c.name as string}</button>
                    ))}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-2">选择档口 *（可多选）</label>
                <div className="flex flex-wrap gap-2">
                  {availableStalls.map(s => (
                    <button key={s.id as string} type="button" onClick={() => toggleStall(s.id as string)} className={`px-3 py-1.5 rounded-lg text-sm border transition ${form.stall_ids.includes(s.id as string) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:border-blue-300'}`}>{s.name as string}</button>
                  ))}
                  {availableStalls.length === 0 && <span className="text-gray-400 text-sm">暂无可选档口</span>}
                </div>
              </div>
            </div>
          )}
          ...
          {editingId && form.role_code === 'CANTEEN_MANAGER' && (
            <div className="mt-4">
              <label className="block text-xs font-medium text-gray-600 mb-1">所属食堂</label>
              <select value={form.org_id} onChange={e => setForm(f => ({ ...f, org_id: e.target.value }))} className="w-full border rounded-lg px-3 py-2 text-sm"><option value="">选择食堂</option>{canteens.map(c => <option key={c.id as string} value={c.id as string}>{c.name as string}</option>)}</select>
            </div>
          )}
          {editingId && form.role_code === 'STALL_MANAGER' && (
            <div className="mt-4">
              <label className="block text-xs font-medium text-gray-600 mb-1">所属档口</label>
              <select value={form.org_id} onChange={e => setForm(f => ({ ...f, org_id: e.target.value }))} className="w-full border rounded-lg px-3 py-2 text-sm"><option value="">选择档口</option>{stalls.map(s => <option key={s.id as string} value={s.id as string}>{s.name as string}</option>)}</select>
            </div>
          )}
          ...
          <table className="w-full text-sm min-w-[500px]">
            ...
            <tbody>{users.map(u => (<tr key={u.id as string} className="border-t hover:bg-gray-50">
              ...
              <td className="px-3 py-2 text-center space-x-1 whitespace-nowrap">
                ...
                <button onClick={() => toggleDisable(u)} className="text-orange-600 hover:underline text-xs">{u.is_disabled ? '启用' : '禁用'}</button>
                <button onClick={() => resetPw(u)} className="text-gray-600 hover:underline text-xs">重置密码</button>
                <button onClick={async () => { if (!confirm('确认删除此账号？删除后不可恢复！')) return; const res = await apiFetch<ApiResp>(`/api/users/${u.id}`, { method: 'DELETE' }); if (res.success) load(); else alert(res.error); }} className="text-red-600 hover:underline text-xs">删除</button>
              </td>
            </tr>))}</tbody>
          </table>
        </div>
...
function SupplierManager() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center"><h3 className="font-semibold text-gray-700">供应商管理</h3><button onClick={() => { setShowForm(!showForm); setEditingId(null); setForm({ name: '', contact_person: '', contact_phone: '', address: '', note: '' }); }} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm">{showForm && !editingId ? '取消' : '新增供应商'}</button></div>
      {showForm && (
        <div className="bg-white rounded-xl shadow p-4 sm:p-5">
  ...
  if (isDeveloper) {
    return (
      <div className="min-h-screen bg-gray-50 flex">
        ...
        <main className="flex-1 p-4 md:p-6 max-w-6xl mx-auto w-full"><DeveloperView user={user} onSwitchAccount={(u) => { setUser(u); setTab('dashboard'); }} /></main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 bg-white border-b z-30 px-4 py-3 flex items-center justify-between">
        <div>
          <h2 className="font-bold text-gray-800 text-sm">食堂管理</h2>
          <p className="text-xs text-gray-400">{[user.company_name, user.org_name].filter(Boolean).join(' / ')}</p>
        </div>
      ...
      {sidebarOpen && (
        <div className="md:hidden fixed left-0 top-0 bottom-0 w-64 bg-white z-50 shadow-xl">
          <div className="p-4 border-b flex justify-between items-center"><h2 className="font-bold text-gray-800">食堂管理</h2><button onClick={() => setSidebarOpen(false)} className="text-gray-400 text-xl">&times;</button></div>
          <nav className="p-2 space-y-1">
            {visibleTabs.map(t => (<button key={t.key} onClick={() => { setTab(t.key); setSidebarOpen(false); }} className={`w-full text-left px-4 py-3 rounded-lg text-sm ${tab === t.key ? 'bg-blue-600 text-white' : 'text-gray-600'}`}>{t.label}</button>))}
            <button onClick={logout} className="w-full text-left px-4 py-3 rounded-lg text-sm text-red-600 hover:bg-red-50">退出登录</button>
          </nav>
        </div>
      )}

      {/* Main content */}
      <main className="flex-1 p-4 md:p-6 mt-16 md:mt-0 max-w-6xl mx-auto w-full">
